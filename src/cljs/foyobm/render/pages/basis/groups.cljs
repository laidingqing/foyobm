(ns foyobm.render.pages.basis.groups
...
(defn- group-list []
  (let [groups @(rf/subscribe [::basis/groups])
        pagination @(rf/subscribe [::basis/groups-pagination])]
    (if (empty? groups)
        [empty-groups]
  ...
  (let [current-company @(rf/subscribe [::auth/current-company])
    ...
    (fn []
      (antd/form {:layout "vertical" :class "w-1/2" :onFinish (fn []
                                                                (rf/dispatch [::basis/create-group @form-state]))}
