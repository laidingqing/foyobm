...
(defn menu-items []
  [{:key "1" :label "首页" :icon (r/as-element [:> HomeOutlined]) :title "首页"}
   ...
   {:key "3" :icon (r/as-element [:> DashboardOutlined]) :label "积分制" :children [
                                                                                ...
                                                                                {:key "3.2" :label "申报积分"}
                                                                                 {:key "3.3" :label "排行榜"}]}
   {:key "4" :icon (r/as-element [:> SettingOutlined]) :label "系统设置" :children [{:key "4.1" :label "积分项目设置" :onClick #(rf/dispatch [::router/push-state :foyobm.render.routes/project-list])}
